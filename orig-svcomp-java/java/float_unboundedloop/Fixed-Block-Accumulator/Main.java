...
public class Main {
  public static void main(String[] args) {
    ...
    double x = 1.0;
    int s = 0; // 0: add, 1: subtract
    int c = 0; // steps taken in current block (length = 2)
    ...
    while (Verifier.nondetBoolean()) {
      ...
      // switch direction after a small fixed block (2 steps)
      if (c == 2) {
        if (s == 0) {
          s = 1;
        } else {
    ...
    assert (x >= -20.0 && x <= 20.0 && s >= 0 && s <= 1 && c >= 0 && c <= 2);
  }
}
