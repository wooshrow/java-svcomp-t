...
public class Main {
  public static void main(String[] args) {
    ...
    while (Verifier.nondetBoolean()) {
      x = x + 1.0; // drift upward (exact in FP)

      if (x > 10.0) x = 10.0; // clamp at the top
      if (x >= 9.0) x = x - 0.5; // gentle backoff near the top

      if (x < 1.0) x = 1.0; // (redundant but keeps the shape symmetric)
    }
